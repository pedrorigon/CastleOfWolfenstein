...
void procura_score(FILE *arq, int comp_media)
{
    Player a;
...
void ordena_score(FILE *arq)
{
    ...
    {
        if(fread(&a, sizeof(Player), 1, arq) == 1)
           for(int i=0; i<(strlen(a.score)-1); i++)
            {
                for(int j=i; j<(strlen(a.score)-1);j++)
                {
